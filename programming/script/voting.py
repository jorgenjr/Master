...
def prepare_vote(CIBERSORT_mixes, LLSR_mixes):
	...
	if (config.VOTE_OPTIONS[config.VOTE] == 'separate'):
		vote_separate(CIBERSORT_result, LLSR_result)
	elif (config.VOTE_OPTIONS[config.VOTE] == 'combined'):
...
def get_coef():
	...
	for i in range(config.CIBERSORT_FIRST_MIX, config.CIBERSORT_LAST_MIX + 1):
		...
		line = linecache.getline(config.PATH + 'Master_files/output/CIBERSORT_tumor_present_0_0', i)
	...
	for i in range(config.ABBAS_FIRST_CELL, config.ABBAS_LAST_CELL + 1):
		line = linecache.getline(config.PATH + 'Master_files/abbas/Abbas_tumor_present_0_0', i)
		splitted_line = line.split('\t')
		line_to_append = []
